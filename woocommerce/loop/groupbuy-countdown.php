...
<?php if ( is_object($product) &&  method_exists( $product, 'get_type') && $product->get_type() == 'groupbuy' ) : ?>
	<?php if (  ( $product->is_closed() === FALSE ) &&  ($product->is_started() === TRUE )) : ?>
		<div class="groupbuy-time" id="countdown-<?php echo $product->get_id() ?>"><?php echo apply_filters('time_text', __( 'До окончания приема заявок осталось:', 'wc_groupbuy' ), $product); ?>
	...
	<?php elseif (( $product->is_closed() === FALSE ) && ($product->is_started() === FALSE )):?>
		...
		<div class="groupbuy-time future" id="countdown-<?php echo $product->get_id() ?>"><?php echo  __( 'Подача заявок на совместную закупку начнется в:', 'wc_groupbuy' ) ?>
